...
**Responsável:** David
**Status:** Correção de gaps — GAP-001 (Espaçamento) em andamento; ver `gaps.md`
...
1. GAP-001 — Espaçamento ajustável (SpacingTheme + toggle + aplicação global)
2. GAP-004 — "Passo X de Y" no Guided Task (correção simples, 1 linha)
3. GAP-003 — `audioFeedbackEnabled` controlar haptics via wrapper
4. GAP-005 — `core/` importar `features/` (mover `UserPreferences` para `core/`)
5. GAP-002 — Notificações locais de lembretes
