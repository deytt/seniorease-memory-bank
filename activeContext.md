...
**Concluído (2026-07-27, Tati — paridade do catálogo Storybook Web):**

- Branch web `fix/storybook-component-parity-2026`, commit `6b808d6`: catálogo reorganizado e alinhado aos componentes reais, removendo stories redundantes/obsoletas e consolidando filtros de tarefas e lembretes.
- Catálogo atualizado para **21 arquivos de stories** mais a introdução em MDX. Novas coberturas: `BackNavigationButton`, `MaskedInput`, `PageHeader`, filtros combinados de Tarefas/Lembretes e `TourHelpButton`.
- O preview global usa o mesmo `globals.css`, as mesmas fontes (`Inter`, `Geist` e `Geist Mono`) e os mesmos tokens semânticos da aplicação.
- Toolbar do Storybook permite validar aparência (claro, escuro, alto contraste e contraste máximo), escala de fonte, densidade de espaçamento, alvos de toque maiores e modo de interface básico/avançado.
- Preferências são aplicadas também ao elemento `<html>`, garantindo que componentes em portals, como dialogs e tooltips, recebam o mesmo contexto visual e de acessibilidade.
- `AppContentDecorator` opcional replica largura máxima e paddings do conteúdo autenticado sem forçar altura de viewport; usado nas stories estruturais do `PageHeader`.
- `PageHeader` documenta composição com `actions`, `tourAction` e comportamento responsivo mobile/desktop. `TourHelpButton` documenta estado padrão, foco por teclado, alto contraste e alvo de toque ampliado.
- Validação concluída com TypeScript, ESLint, `git diff --check` e build estático do Storybook. Apenas avisos não bloqueantes de tamanho dos bundles permanecem.
