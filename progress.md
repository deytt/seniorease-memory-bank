...
- [x] Persistência web de passos alinhada ao mobile na subcollection `tasks/{taskId}/steps/{stepId}`; leitura compatível com array legado durante a transição (2026-07-21) — **revertida** (issue #81 itens 1/2/5)
- [x] **ADR-023:** contrato único `steps` como campo array em `tasks/{taskId}` (Web + Mobile); sub-collection marcada como legado (2026-07-22)
- [x] Issue web #81 itens 1/2/5: `FirebaseTaskRepository` voltou a ler/gravar `steps` como array no documento (sub-collection bloqueada nas rules); corrige carregar/criar tarefas e “Carregar exemplos” (2026-07-25)
- [x] Filtros de tarefas no padrão dos lembretes: `TaskFilterSheet` + chips ativos removíveis + badge de contagem no botão Filtrar (2026-07-25)
...
- [x] Issue web #81 item 11: toast de sucesso legível no dark mode (`--toast-*-text` claro) + tipografia em rem alinhada à escala de acessibilidade (branch `fix/issue-81-bugs-melhorias`, 2026-07-25)
...
- [x] Card de lembrete: swipe bidirecional (esquerda apaga / direita edita; concluídos só permitem excluir), toque para expandir a descrição completa (acessível), data no card e dica de swipe ("peek") na 1ª abertura da sessão
